...
```{r}
viirs_old_esr<-readRDS("Data/viirs_old_esr.RDS")
#Aleutians filters (no depth)
viirs_ai<-viirs_old_esr%>%filter(Ecosystem_Area=="Aleutian Islands" & WATERS_COD=="FED")
#BSGOA filters (includes depth)
viirs_bsgoa<-viirs_old_esr%>%filter(Ecosystem_Area=="Eastern Bering Sea" |  Ecosystem_Area=="Gulf of Alaska")%>%
...
#averages
viirs_old_avg<-viirs_old_esr%>%group_by(Ecosystem_Subarea, year, week)%>%
...
#save
viirs_old_avg%>%saveRDS("Data/viirs_old_av.RDS")
```
...
#combine
viirs_2022<-viirs_ai%>%bind_rows(viirs_bsgoa)
#remove old ones
rm(viirs_ai);rm(viirs_bsgoa)
...
#averages
viirs_2022_avg<-viirs_2022%>%
  group_by(Ecosystem_Subarea, week)%>%
  summarise(chlorophyll=mean(chlorophyll, na.rm=T),
            n=n())

#save
viirs_2022_avg%>%saveRDS("Data/viirs_2022_avg.RDS")
```